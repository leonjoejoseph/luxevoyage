...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Chat feedback endpoint - sends chat history to luxevoyage25@gmail.com
  app.post("/api/chat/feedback", async (req, res) => {
    try {
      const { rating, feedback, messages } = req.body;

      // Always send chat history to luxevoyage25@gmail.com
      const result = await sendContactEmail({
        firstName: "Chat",
        lastName: "History",
        email: "luxevoyage25@gmail.com",
        message: `
LuxeVoyage Chat Session
======================
...
Chat Messages:
${messages?.map((msg: any, i: number) => `${i + 1}. ${msg.sender.toUpperCase()}: ${msg.text}`).join('\n') || 'No messages recorded'}
